...
import { getAllNodesFlat, clearAllNodes, putAllNodesFlat, upsertNodesFlatLww } from '../db';
...
export function SyncManager() {
  ...
  const loadCloudDataSilently = useCallback(async () => {
    ...
    try {
      ...
      if (hasChanges) {
        await upsertNodesFlatLww(changedNodes);
  ...
  const processChangeLogRows = useCallback(
    async (userId: string, changes: SyncChangeLogRow[]): Promise<{ didFullSync: boolean }> => {
      ...
      if (nodesToUpsert.length > 0) {
        await upsertNodesFlatLww(nodesToUpsert);
        notifyDataUpdated();
      }
  ...
  const handleFirstSync = useCallback(async (isNewLogin: boolean) => {
    try {
      ...
      if (hasCloudData) {
        ...
        try {
          await clearAllNodes();
          await putAllNodesFlat(mergedNodes);
  ...
  const handleChooseLocal = async () => {
    try {
      ...
      setShowConflictDialog(false);
      const syncToastId = showToast('Синхронизация с облаком…', undefined, {
        isLoading: true,
        persistent: true,
      });

      try {
        const freshLocal = await getAllNodesFlat();
        const nodesToPush = freshLocal.length >= localNodes.length ? freshLocal : localNodes;
        await syncAllNodesToFirestore(nodesToPush, undefined, {
          purgeCloudOrphans: true,
          overwriteAll: true,
        });
        removeToast(syncToastId);
        showToast('Локальные данные сохранены в облако', undefined, { type: 'success' });
        log('[handleChooseLocal] Local data mirrored to cloud (orphans purged)');
      } catch (error) {
        removeToast(syncToastId);
        log('[handleChooseLocal] Error syncing local data:', error);
        console.error('[handleChooseLocal] Error syncing local data:', error);
        showToast('Ошибка синхронизации', undefined, { type: 'error' });
      }
    } catch (error) {
  ...
  const handleChooseCloud = async () => {
    try {
      ...
      await clearAllNodes();
      await putAllNodesFlat(cloudNodes);
