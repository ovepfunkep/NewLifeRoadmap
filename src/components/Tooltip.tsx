...
export function Tooltip({ text, children, position = 'top', multiline = false }: TooltipProps) {
  ...
  return (
    <div className="relative group">
      {children}
      <div className={`absolute ${positionClasses[position]} px-2 py-1 text-xs text-white bg-gray-900 dark:bg-gray-700 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none ${multiline ? 'whitespace-normal min-w-[150px] max-w-[min(90vw,20rem)] text-left' : 'whitespace-nowrap'} z-[9999]`}>
        {text}
