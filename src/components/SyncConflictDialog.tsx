import { useState, useEffect, useRef, useMemo } from 'react';
import { Node } from '../types';
import { SyncDiff, compareNodes } from '../utils/syncCompare';
import {
  formatNodeDiffFieldValue,
  getDisplayDiffFields,
  type NodeDiffFieldKey,
} from '../utils/syncConflictDisplay';
import { useTranslation } from '../i18n';
import { useLanguage } from '../contexts/LanguageContext';
import { Z_MODAL } from '../config/zLayers';

interface SyncConflictDialogProps {
  localNodes: Node[];
  cloudNodes: Node[];
  onChooseLocal: () => void;
  onChooseCloud: () => void;
  onMerge: () => void;
  onCancel: () => void;
}

const FIELD_LABEL_KEYS: Record<NodeDiffFieldKey, string> = {
  title: 'syncConflict.fieldTitle',
  description: 'syncConflict.fieldDescription',
  completed: 'syncConflict.fieldCompleted',
  deadline: 'syncConflict.fieldDeadline',
  deadlineEnd: 'syncConflict.fieldDeadlineEnd',
  isRecurring: 'syncConflict.fieldIsRecurring',
  recurrence: 'syncConflict.fieldRecurrence',
  priority: 'syncConflict.fieldPriority',
  parentId: 'syncConflict.fieldParentId',
  deletedAt: 'syncConflict.fieldDeletedAt',
};

/**
 * Построить дерево из плоского списка узлов
 */
function buildTreeFromFlatList(nodes: Node[]): Node | null {
  const nodeMap = new Map<string, Node>();

  nodes.forEach(node => {
    nodeMap.set(node.id, { ...node, children: [] });
  });

  let root: Node | null = null;

  nodes.forEach(node => {
    const nodeInMap = nodeMap.get(node.id)!;

    if (!node.parentId || node.id === 'root-node' || !nodeMap.has(node.parentId)) {
      if (!root || node.id === 'root-node') {
        root = nodeInMap;
      }
    } else {
      const parent = nodeMap.get(node.parentId);
      if (parent) {
        parent.children.push(nodeInMap);
      }
    }
  });

  return root;
}

/**
 * Оставить только узлы, которые реально могут отображаться в UI:
 * - все активные
 * - удаленные, если они есть только на одной стороне
 * - удаленные/активные узлы с расхождением статуса удаления
 * - плюс их предки (чтобы ветка была видимой)
 */
function filterVisibleNodes(nodes: Node[], diff: SyncDiff, source: 'local' | 'cloud'): Node[] {
  const nodeMap = new Map<string, Node>(nodes.map(n => [n.id, { ...n, children: [] }]));
  const keepIds = new Set<string>();

  const isLocalOnly = (id: string) => diff.localOnly.some(n => n.id === id);
  const isCloudOnly = (id: string) => diff.cloudOnly.some(n => n.id === id);
  const isDeleteMismatch = (id: string) => diff.different.some(d => d.nodeId === id && d.differences.includes('deletedAt'));

  nodes.forEach(node => {
    if (!node.deletedAt) {
      keepIds.add(node.id);
      return;
    }
    if (source === 'local' && isLocalOnly(node.id)) {
      keepIds.add(node.id);
      return;
    }
    if (source === 'cloud' && isCloudOnly(node.id)) {
      keepIds.add(node.id);
      return;
    }
    if (isDeleteMismatch(node.id)) {
      keepIds.add(node.id);
    }
  });

  const addParents = (id: string) => {
    let curr = nodeMap.get(id)?.parentId || null;
    while (curr) {
      if (keepIds.has(curr)) break;
      keepIds.add(curr);
      curr = nodeMap.get(curr)?.parentId || null;
    }
  };
  Array.from(keepIds).forEach(addParents);

  const keptNodes = Array.from(keepIds).map(id => nodeMap.get(id)!).filter(Boolean);
  return keptNodes;
}

interface NodeFieldDiffPanelProps {
  node: Node;
  fields: NodeDiffFieldKey[];
  formatValue: (node: Node, field: NodeDiffFieldKey) => string;
  fieldLabel: (field: NodeDiffFieldKey) => string;
  panelTitle: string;
}

function NodeFieldDiffPanel({
  node,
  fields,
  formatValue,
  fieldLabel,
  panelTitle,
}: NodeFieldDiffPanelProps) {
  return (
    <div className="mx-2 mb-2 rounded-lg bg-yellow-50/80 px-3 py-2 text-xs dark:bg-yellow-900/20">
      <div className="mb-1.5 font-semibold text-yellow-800 dark:text-yellow-200">{panelTitle}</div>
      <ul className="space-y-1.5">
        {fields.map((field) => (
          <li key={field}>
            <span className="font-medium text-gray-600 dark:text-gray-400">{fieldLabel(field)}: </span>
            <span className="text-gray-800 dark:text-gray-200 break-words">{formatValue(node, field)}</span>
          </li>
        ))}
      </ul>
    </div>
  );
}

interface TreeNodeProps {
  node: Node;
  level: number;
  diff: SyncDiff;
  source: 'local' | 'cloud';
  expandedNodes: Set<string>;
  toggleExpand: (nodeId: string) => void;
  titleById: Map<string, string>;
  locale: string;
  t: (key: string) => string;
}

function TreeNode({
  node,
  level,
  diff,
  source,
  expandedNodes,
  toggleExpand,
  titleById,
  locale,
  t,
}: TreeNodeProps) {
  const isExpanded = expandedNodes.has(node.id);
  const isLocalOnly = diff.localOnly.some(n => n.id === node.id);
  const isCloudOnly = diff.cloudOnly.some(n => n.id === node.id);
  const diffEntry = diff.different.find(d => d.nodeId === node.id);
  const isDeleted = !!node.deletedAt;
  const displayFields =
    diffEntry != null
      ? getDisplayDiffFields(diffEntry.local, diffEntry.cloud)
      : [];
  const isChanged = displayFields.length > 0;
  const sideNode =
    diffEntry && isChanged
      ? source === 'local'
        ? diffEntry.local
        : diffEntry.cloud
      : node;
  const onlyOnThisSide =
    (isLocalOnly && source === 'local' && !isCloudOnly) ||
    (isCloudOnly && source === 'cloud' && !isLocalOnly);

  const formatValue = (n: Node, field: NodeDiffFieldKey) => {
    const raw = formatNodeDiffFieldValue(n, field, locale, titleById);
    if (raw === 'yes') return t('syncConflict.yes');
    if (raw === 'no') return t('syncConflict.no');
    return raw;
  };

  const nodeHasVisibleDiff = (n: Node): boolean => {
    const localOnly = diff.localOnly.some(item => item.id === n.id && !item.deletedAt);
    const cloudOnly = diff.cloudOnly.some(item => item.id === n.id && !item.deletedAt);
    if (localOnly || cloudOnly) return true;
    const change = diff.different.find(item => item.nodeId === n.id);
    if (change && getDisplayDiffFields(change.local, change.cloud).length > 0) {
      return true;
    }
    return false;
  };

  const hasDiffInSubtree = (n: Node): boolean => {
    if (nodeHasVisibleDiff(n)) return true;
    return (n.children || []).some(child => hasDiffInSubtree(child));
  };

  if (!hasDiffInSubtree(node)) return null;

  const visibleChildren = (node.children || []).filter(child => hasDiffInSubtree(child));
  const hasFieldPanel = displayFields.length > 0;
  const canExpand = visibleChildren.length > 0 || hasFieldPanel || onlyOnThisSide;

  let badge = null;
  if (isDeleted) {
    badge = (
      <span className="text-xs px-2 py-0.5 rounded bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300">
        {t('syncConflict.badgeDeleted')}
      </span>
    );
  } else if (isLocalOnly && source === 'local') {
    badge = (
      <span className="text-xs px-2 py-0.5 rounded bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300">
        {t('syncConflict.badgeNew')}
      </span>
    );
  } else if (isCloudOnly && source === 'cloud') {
    badge = (
      <span className="text-xs px-2 py-0.5 rounded bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300">
        {t('syncConflict.badgeNew')}
      </span>
    );
  } else if (isChanged) {
    badge = (
      <span className="text-xs px-2 py-0.5 rounded bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300">
        {t('syncConflict.badgeChanged')}
      </span>
    );
  }

  const fieldLabel = (field: NodeDiffFieldKey) => t(FIELD_LABEL_KEYS[field]);

  return (
    <div>
      <div
        className="flex items-center gap-2 py-1.5 px-2 rounded-lg cursor-pointer transition-colors hover:bg-accent/5 group"
        onClick={() => canExpand && toggleExpand(node.id)}
      >
        <div
          className="w-5 h-5 flex items-center justify-center rounded-md hover:bg-accent/10 transition-colors font-bold text-xs"
          style={{ color: 'var(--accent)' }}
        >
          {canExpand ? (isExpanded ? '−' : '+') : <span className="w-4" />}
        </div>
        <span
          className={`flex-1 text-sm truncate group-hover:text-accent transition-colors ${isDeleted ? 'text-gray-400 line-through' : 'text-gray-900 dark:text-gray-100'}`}
        >
          {node.title}
        </span>
        {badge}
      </div>
      {isExpanded && (
        <>
          {hasFieldPanel && (
            <NodeFieldDiffPanel
              node={sideNode}
              fields={displayFields}
              formatValue={formatValue}
              fieldLabel={fieldLabel}
              panelTitle={t('syncConflict.diffFieldsTitle')}
            />
          )}
          {onlyOnThisSide && !hasFieldPanel && (
            <p className="mx-2 mb-2 px-3 py-2 text-xs text-gray-500 dark:text-gray-400 rounded-lg bg-gray-100/80 dark:bg-gray-800/50">
              {t('syncConflict.onlyOnSide')}
            </p>
          )}
          {visibleChildren.length > 0 && (
            <div className="ml-5 border-l border-gray-200 dark:border-gray-700">
              {visibleChildren.map((child) => (
                <TreeNode
                  key={child.id}
                  node={child}
                  level={level + 1}
                  diff={diff}
                  source={source}
                  expandedNodes={expandedNodes}
                  toggleExpand={toggleExpand}
                  titleById={titleById}
                  locale={locale}
                  t={t}
                />
              ))}
            </div>
          )}
        </>
      )}
    </div>
  );
}

export function SyncConflictDialog({
  localNodes,
  cloudNodes,
  onChooseLocal,
  onChooseCloud,
  onMerge,
  onCancel,
}: SyncConflictDialogProps) {
  const t = useTranslation();
  const { language } = useLanguage();
  const locale = language === 'ru' ? 'ru-RU' : 'en-US';

  const [diff, setDiff] = useState<SyncDiff | null>(null);
  const [localRoot, setLocalRoot] = useState<Node | null>(null);
  const [cloudRoot, setCloudRoot] = useState<Node | null>(null);
  const [activeTab, setActiveTab] = useState<'local' | 'cloud'>('local');
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set(['root-node']));
  const modalRef = useRef<HTMLDivElement>(null);
  const localScrollRef = useRef<HTMLDivElement>(null);
  const cloudScrollRef = useRef<HTMLDivElement>(null);
  const isScrollingRef = useRef<boolean>(false);
  const clickStartRef = useRef<{ target: EventTarget | null; inside: boolean } | null>(null);
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;

  const titleById = useMemo(() => {
    const map = new Map<string, string>();
    for (const n of [...localNodes, ...cloudNodes]) {
      map.set(n.id, n.title);
    }
    return map;
  }, [localNodes, cloudNodes]);

  useEffect(() => {
    const calculateDiff = () => {
      const localFlat = localNodes.map(n => ({ ...n, children: [] }));
      const cloudFlat = cloudNodes.map(n => ({ ...n, children: [] }));
      const comparison = compareNodes(localFlat, cloudFlat);
      setDiff(comparison);

      const visibleLocal = filterVisibleNodes(localNodes, comparison, 'local');
      const visibleCloud = filterVisibleNodes(cloudNodes, comparison, 'cloud');
      setLocalRoot(buildTreeFromFlatList(visibleLocal));
      setCloudRoot(buildTreeFromFlatList(visibleCloud));

      const autoExpand = new Set<string>(['root-node']);
      const findAndExpandParents = (nodeId: string | null) => {
        let curr = nodeId;
        while (curr) {
          autoExpand.add(curr);
          const p = visibleLocal.find(x => x.id === curr) || visibleCloud.find(x => x.id === curr);
          curr = p?.parentId || null;
        }
      };

      comparison.localOnly.forEach(n => !n.deletedAt && findAndExpandParents(n.parentId));
      comparison.cloudOnly.forEach(n => !n.deletedAt && findAndExpandParents(n.parentId));
      comparison.different.forEach(d => {
        if (getDisplayDiffFields(d.local, d.cloud).length > 0) {
          autoExpand.add(d.nodeId);
          findAndExpandParents(d.local.parentId ?? d.local.id);
        }
      });
      setExpandedNodes(autoExpand);
    };
    calculateDiff();
  }, [localNodes, cloudNodes]);

  const toggleExpand = (nodeId: string) => {
    setExpandedNodes(prev => {
      const next = new Set(prev);
      if (next.has(nodeId)) next.delete(nodeId);
      else next.add(nodeId);
      return next;
    });
  };

  const handleScroll = (source: 'local' | 'cloud') => {
    if (isScrollingRef.current) return;
    isScrollingRef.current = true;
    const sourceEl = source === 'local' ? localScrollRef.current : cloudScrollRef.current;
    const targetEl = source === 'local' ? cloudScrollRef.current : localScrollRef.current;
    if (sourceEl && targetEl) targetEl.scrollTop = sourceEl.scrollTop;
    setTimeout(() => { isScrollingRef.current = false; }, 20);
  };

  if (!diff || !localRoot || !cloudRoot) return null;

  const treeProps = { diff, expandedNodes, toggleExpand, titleById, locale, t };

  return (
    <div
      className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-md p-4"
      style={{ zIndex: Z_MODAL }}
      onMouseDown={(e) => {
        clickStartRef.current = {
          target: e.target,
          inside: modalRef.current?.contains(e.target as any) || false
        };
      }}
      onClick={(e) => {
        if (clickStartRef.current && !clickStartRef.current.inside && !modalRef.current?.contains(e.target as any)) {
          onCancel();
        }
      }}
    >
      <div
        ref={modalRef}
        className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-gray-800 lg:rounded-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 bg-accent/5">
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-1" style={{ color: 'var(--accent)' }}>
            {t('syncConflict.title')}
          </h2>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {t('syncConflict.subtitle')}
          </p>
        </div>

        {isMobile && (
          <div className="flex border-b border-gray-200 dark:border-gray-700">
            <button
              onClick={() => setActiveTab('local')}
              className={`flex-1 py-3 text-xs font-bold ${activeTab === 'local' ? 'text-accent border-b-2 border-accent' : 'text-gray-500'}`}
            >
              {t('syncConflict.onDevice')}
            </button>
            <button
              onClick={() => setActiveTab('cloud')}
              className={`flex-1 py-3 text-xs font-bold ${activeTab === 'cloud' ? 'text-accent border-b-2 border-accent' : 'text-gray-500'}`}
            >
              {t('syncConflict.inCloud')}
            </button>
          </div>
        )}

        <div className="flex-1 p-6 overflow-hidden flex flex-col min-h-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full min-h-0">
            {(!isMobile || activeTab === 'local') && (
              <div className="flex flex-col h-full min-h-0">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-3">
                  {t('syncConflict.onDevice')}
                </h3>
                <div
                  ref={localScrollRef}
                  onScroll={() => handleScroll('local')}
                  className="flex-1 overflow-y-auto rounded-xl bg-gray-50 p-4 shadow-sm dark:bg-gray-900/50 custom-scrollbar"
                >
                  <TreeNode node={localRoot} level={0} source="local" {...treeProps} />
                </div>
              </div>
            )}
            {(!isMobile || activeTab === 'cloud') && (
              <div className="flex flex-col h-full min-h-0">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-3">
                  {t('syncConflict.inCloud')}
                </h3>
                <div
                  ref={cloudScrollRef}
                  onScroll={() => handleScroll('cloud')}
                  className="flex-1 overflow-y-auto rounded-xl bg-gray-50 p-4 shadow-sm dark:bg-gray-900/50 custom-scrollbar"
                >
                  <TreeNode node={cloudRoot} level={0} source="cloud" {...treeProps} />
                </div>
              </div>
            )}
          </div>
        </div>

        <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <button
              onClick={onMerge}
              className="w-full sm:w-auto px-8 py-3 rounded-xl bg-accent text-white font-bold shadow-lg shadow-accent/20 hover:brightness-110 active:scale-95 transition-all text-sm"
            >
              {t('syncConflict.merge')}
            </button>
            <button
              onClick={onChooseLocal}
              className="w-full sm:w-auto px-6 py-3 rounded-xl border border-blue-200 dark:border-blue-800 text-blue-600 dark:text-blue-400 font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 active:scale-95 transition-all text-sm"
            >
              {t('syncConflict.keepLocal')}
            </button>
            <button
              onClick={onChooseCloud}
              className="w-full sm:w-auto px-6 py-3 rounded-xl border border-green-200 dark:border-green-800 text-green-600 dark:text-green-400 font-bold hover:bg-green-50 dark:hover:bg-green-900/20 active:scale-95 transition-all text-sm"
            >
              {t('syncConflict.takeCloud')}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
