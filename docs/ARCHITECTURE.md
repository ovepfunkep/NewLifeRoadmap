...
- **Bulk push** (`syncAllNodesToFirestore`) should receive an in-memory **cloud snapshot** when you already loaded it, so the function does not run a second full `getDocs` for diff-only writes. If the cloud is **known empty** (first login, no remote nodes), pass **`[]`** as the snapshot so the client skips the expensive internal fetch.
